...
- **Never exceed the Fix-in-PR allowance (25 per PR).** It's counted across every review round the PR goes through and every pass within each round. If candidates exceed what's left, rank by Priority then risk-removed-per-token and demote the rest to Follow-up.
- **An item `senior-engineer` skips as much costlier than briefed** is demoted to Follow-up, filed in its grouped issue, and still counts toward `Fix-in-PR used`. Pass the list of skipped items to `code-reviewer` in the next pass's brief so it isn't re-reported as a failed fix. **This skip route applies only to Fix-in-PR items — `senior-engineer` may never skip a Blocking finding.** If a Blocking finding turns out much costlier than briefed, `senior-engineer` reports it instead; the workflow posts it as a Decision Needed PR comment and waits for the reply, using the same "Fix now"/"Defer" replies as the post-pass-4 case.
...
### Gate Status
[PASS / BLOCKED — with reason if blocked; PASS requires zero open Blocking findings (a post-pass-4 Blocking finding counts as closed once resolved through its Decision Needed reply) and no open Fix-in-PR items; BLOCKED never means a Follow-up or an unresolved Decision Needed one]
```
