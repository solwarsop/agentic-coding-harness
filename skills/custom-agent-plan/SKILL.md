...
3. **Wait for a new comment on the PR before proceeding** — check with `gh pr view <PR> --json comments` (or `gh api repos/:owner/:repo/issues/:number/comments`) for anything posted after the Plan comment. If the user invoked this whole task via `/loop` (so a `/loop` prompt to re-fire already exists), poll every 10-20 minutes rather than blocking the conversation. **Never call `ScheduleWakeup` directly to self-pace outside of an active `/loop`** — it's a `/loop` dynamic-mode primitive that requires a `prompt` to re-fire (the tool errors with "`prompt` is required when `stop` is not true" if one isn't supplied), and no such prompt exists unless `/loop` is already driving this task. Outside of `/loop`, ask the user to say when they've commented, then re-check.
